...
func (s *Sqlite) scanOAuth2Client(scan database.Scanner, includeCount bool) (*models.OAuth2Client, uint64, error) {
	var (
...
func (s *Sqlite) scanOAuth2Clients(rows database.ResultIterator) ([]*models.OAuth2Client, uint64, error) {
	var (
	...
	for rows.Next() {
		client, c, err := s.scanOAuth2Client(rows, true)
		if err != nil {
			return nil, 0, err
		}
...
func (s *Sqlite) GetOAuth2ClientByClientID(ctx context.Context, clientID string) (*models.OAuth2Client, error) {
	...
	client, _, err := s.scanOAuth2Client(row, false)
	return client, err
}
...
func (s *Sqlite) GetAllOAuth2Clients(ctx context.Context) ([]*models.OAuth2Client, error) {
	...
	list, _, err := s.scanOAuth2Clients(rows)
	if err != nil {
...
func (s *Sqlite) GetAllOAuth2ClientsForUser(ctx context.Context, userID uint64) ([]*models.OAuth2Client, error) {
	query, args := s.buildGetOAuth2ClientsQuery(userID, nil)
...
func (s *Sqlite) buildGetOAuth2ClientsQuery(userID uint64, filter *models.QueryFilter) (query string, args []interface{}) {
	var err error
...
func (s *Sqlite) GetOAuth2ClientsForUser(ctx context.Context, userID uint64, filter *models.QueryFilter) (*models.OAuth2ClientList, error) {
	query, args := s.buildGetOAuth2ClientsQuery(userID, filter)
	rows, err := s.db.QueryContext(ctx, query, args...)
	...
	list, count, err := s.scanOAuth2Clients(rows)
	if err != nil {

...
func (s *Sqlite) scanWebhook(scan database.Scanner, includeCount bool) (*models.Webhook, uint64, error) {
	var (
		x     = &models.Webhook{}
		count uint64
		eventsStr,
...
func (s *Sqlite) scanWebhooks(rows database.ResultIterator) ([]models.Webhook, uint64, error) {
	var (
		list  []models.Webhook
		count uint64
	)
	...
	for rows.Next() {
		webhook, c, err := s.scanWebhook(rows, true)
		if err != nil {
			return nil, 0, err
		}
...
func (s *Sqlite) GetWebhook(ctx context.Context, webhookID, userID uint64) (*models.Webhook, error) {
	...
	webhook, _, err := s.scanWebhook(row, false)
	if err != nil {
...
func (s *Sqlite) GetAllWebhooks(ctx context.Context) (*models.WebhookList, error) {
	...
	list, count, err := s.scanWebhooks(rows)
	if err != nil {
...
func (s *Sqlite) buildGetWebhooksQuery(userID uint64, filter *models.QueryFilter) (query string, args []interface{}) {
	...
	builder := s.sqlBuilder.
		Select(append(webhooksTableColumns, fmt.Sprintf("(%s)", s.buildGetAllWebhooksCountQuery()))...).
		From(webhooksTableName).
		Where(squirrel.Eq{
			fmt.Sprintf("%s.%s", webhooksTableName, webhooksTableOwnershipColumn): userID,
			fmt.Sprintf("%s.archived_on", webhooksTableName):                      nil,

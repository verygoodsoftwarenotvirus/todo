<script lang="typescript">
  ...
  let logger = new Logger().withDebugValue("source", "src/components/AuditLogTable/AuditLogTable.svelte");

  // set up translations
  let currentSessionSettings = new UserSiteSettings();
  let translationsToUse = translations.messagesFor(currentSessionSettings.language).components.auditLogEntryTable;
  const unsubscribeFromSettingsUpdates = sessionSettingsStore.subscribe((value: UserSiteSettings) => {
    currentSessionSettings = value;
    translationsToUse = translations.messagesFor(currentSessionSettings.language).components.auditLogEntryTable;
  });
  onDestroy(unsubscribeFromSettingsUpdates);

  function fetchEntries() {
    entryFetchFunc.then((response: AxiosResponse<AuditLogEntry[]>) => {
    ...
    <div class="flex flex-wrap items-center">
      ...
      <div class="text-center">
        <div class="px-4 py-2 m-2">
          <button on:click={console.log('decrementPageFunction')} disabled={decrementDisabled}><i class="fa fa-arrow-circle-left"></i></button>
          &nbsp;
          {#if queryFilter.page > 0 }
            {"translationsToUse.page"} {queryFilter.page}
          {/if}
          &nbsp;
          <button on:click={console.log('incrementPageFunction')} disabled={incrementDisabled}><i class="fa fa-arrow-circle-right"></i></button>
        </div>
      </div>
      ...
      <div class="flex border-grey-light border">
        <input class="w-full rounded ml-1" type="text" placeholder={"translationsToUse.inputPlaceholders.search"} bind:value={searchQuery} on:keyup={console.log("search")}>
      </div>

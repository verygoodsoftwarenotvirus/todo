    ...
    V1APIClient.deleteWebhook(id)
      .then((response: AxiosResponse<Webhook>) => {
        if (response.status === 204) {
          ...
          navigate('/things/webhooks', { state: {}, replace: true });
        }
      })
      .catch((error: AxiosError) => {
        if (error.response) {
      ...
      <div class="flex flex-wrap webhooks-center">
        ...
        <div class="flex w-full max-w-full flex-grow justify-end flex-1">
          <button
            ...
            on:click={saveWebhook}><i class="fa fa-save" />
            Save</button>
        </div>
      </div>
    </div>
    <div>
      <div class="flex flex-wrap -mx-3 mb-6">
        ...
        <div class="w-full md:w-1/2 px-3">
          ...
          <input class="appearance-none block w-full text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" type="text" on:keyup={evaluateChanges} bind:value={webhook.details}>
        </div>
        <div
          class="flex w-full mr-3 mt-4 max-w-full flex-grow justify-end flex-1">
          <button
            class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
            on:click={deleteWebhook}><i class="fa fa-trash-alt" />
            Delete</button>
        </div>
      </div>
    </div>

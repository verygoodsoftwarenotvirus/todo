...
const logger = new Logger().withDebugValue("source", "src/stores/user_status_store.ts");
const localStorageKey = "userStatus"

function buildUserStatusStore() {
    const storedUserStatus: UserStatus = JSON.parse(localStorage.getItem(localStorageKey) || '{}') as UserStatus;

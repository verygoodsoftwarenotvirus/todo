...
func (s *Service) SetupRoutes(router routing.Router) {
	...
	router.WithMiddleware(s.authService.AdminMiddleware).Get("/admin/users/search", s.buildUsersTableView(true, true))
	router.WithMiddleware(s.authService.AdminMiddleware).Get("/dashboard_pages/admin/users/search", s.buildUsersTableView(false, true))
	router.WithMiddleware(s.authService.AdminMiddleware).Get("/admin/users", s.buildUsersTableView(true, false))
	router.WithMiddleware(s.authService.AdminMiddleware).Get("/dashboard_pages/admin/users", s.buildUsersTableView(false, false))

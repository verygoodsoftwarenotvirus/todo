...
var (
	_ querybuilding.AuditLogEntrySQLQueryBuilder = (*MySQL)(nil)
)

// BuildGetAuditLogEntryQuery constructs a SQL query for fetching an audit log entry with a given ID.
func (b *MySQL) BuildGetAuditLogEntryQuery(ctx context.Context, entryID string) (query string, args []interface{}) {
...
func (b *MySQL) BuildGetAllAuditLogEntriesCountQuery(ctx context.Context) string {
	_, span := b.tracer.StartSpan(ctx)
	defer span.End()
...
func (b *MySQL) BuildGetBatchOfAuditLogEntriesQuery(ctx context.Context, beginID, endID uint64) (query string, args []interface{}) {
	_, span := b.tracer.StartSpan(ctx)
	defer span.End()
...
func (b *MySQL) BuildGetAuditLogEntriesQuery(ctx context.Context, filter *types.QueryFilter) (query string, args []interface{}) {
	_, span := b.tracer.StartSpan(ctx)
	defer span.End()
...
func (b *MySQL) BuildCreateAuditLogEntryQuery(ctx context.Context, input *types.AuditLogEntryCreationInput) (query string, args []interface{}) {
	_, span := b.tracer.StartSpan(ctx)
	defer span.End()

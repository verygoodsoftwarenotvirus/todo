...
func TestPostgres_BuildGetUserQuery(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		expectedQuery := "SELECT users.id, users.external_id, users.username, users.avatar_src, users.hashed_password, users.requires_password_change, users.password_last_changed_on, users.two_factor_secret, users.two_factor_secret_verified_on, users.service_role, users.reputation, users.reputation_explanation, users.created_on, users.last_updated_on, users.archived_on FROM users WHERE users.archived_on IS NULL AND users.id = $1 AND users.two_factor_secret_verified_on IS NOT NULL"
		expectedArgs := []interface{}{
			exampleUser.ID,
		}
...
func TestPostgres_BuildTestUserCreationQuery(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		expectedQuery := "INSERT INTO users (external_id,username,hashed_password,two_factor_secret,reputation,service_role,two_factor_secret_verified_on) VALUES ($1,$2,$3,$4,$5,$6,extract(epoch FROM NOW())) RETURNING id"
		expectedArgs := []interface{}{
...
func TestPostgres_BuildCreateUserQuery(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		expectedQuery := "INSERT INTO users (external_id,username,hashed_password,two_factor_secret,reputation,service_role) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id"
		expectedArgs := []interface{}{
			exampleUser.ExternalID,
			exampleUser.Username,

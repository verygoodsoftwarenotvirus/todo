...
func TestSqlite_BuildUpdateAccountQuery(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		expectedQuery := "UPDATE accounts SET name = ?, contact_email = ?, contact_phone = ?, last_updated_on = (strftime('%s','now')) WHERE archived_on IS NULL AND belongs_to_user = ? AND id = ?"
		expectedArgs := []interface{}{
			exampleAccount.Name,
			exampleAccount.ContactEmail,
			exampleAccount.ContactPhone,

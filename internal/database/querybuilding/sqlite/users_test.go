...
func TestSqlite_BuildTestUserCreationQuery(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		expectedQuery := "INSERT INTO users (id,username,hashed_password,two_factor_secret,reputation,service_roles,two_factor_secret_verified_on) VALUES (?,?,?,?,?,?,(strftime('%s','now')))"
		expectedArgs := []interface{}{
			exampleInput.ID,
...
func TestSqlite_BuildGetAuditLogEntriesForUserQuery(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		expectedQuery := "SELECT audit_log.id, audit_log.event_type, audit_log.context, audit_log.created_on FROM audit_log WHERE (json_extract(audit_log.context, '$.user_id') = ? OR json_extract(audit_log.context, '$.performed_by') = ?) ORDER BY audit_log.created_on"
		expectedArgs := []interface{}{
			exampleUser.ID,
			exampleUser.ID,
		}

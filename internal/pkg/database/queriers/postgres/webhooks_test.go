...
func TestPostgres_GetAllWebhooksCount(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		actual, err := q.GetAllWebhooksCount(ctx)
		assert.NoError(t, err)
...
func TestPostgres_buildGetWebhooksQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "SELECT webhooks.id, webhooks.name, webhooks.content_type, webhooks.url, webhooks.method, webhooks.events, webhooks.data_types, webhooks.topics, webhooks.created_on, webhooks.last_updated_on, webhooks.archived_on, webhooks.belongs_to_user, (SELECT COUNT(*) FROM webhooks WHERE webhooks.archived_on IS NULL AND webhooks.belongs_to_user = $1) FROM webhooks WHERE webhooks.archived_on IS NULL AND webhooks.belongs_to_user = $2 AND webhooks.created_on > $3 AND webhooks.created_on < $4 AND webhooks.last_updated_on > $5 AND webhooks.last_updated_on < $6 ORDER BY webhooks.created_on LIMIT 20 OFFSET 180"
		expectedArgs := []interface{}{
			exampleUser.ID,
			filter.CreatedAfter,
...
func TestPostgres_GetWebhooks(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		actual, err := q.GetWebhooks(ctx, exampleUser.ID, filter)
		assert.NoError(t, err)
...
func TestPostgres_CreateWebhook(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		actual, err := q.CreateWebhook(ctx, exampleInput)
		assert.NoError(t, err)
		assert.Equal(t, exampleWebhook, actual)
...
func TestPostgres_UpdateWebhook(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		err := q.UpdateWebhook(ctx, exampleWebhook)
		assert.NoError(t, err)
...
func TestPostgres_ArchiveWebhook(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		err := q.ArchiveWebhook(ctx, exampleWebhook.ID, exampleWebhook.BelongsToUser)
		assert.NoError(t, err)
...
func TestPostgres_LogWebhookArchiveEvent(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		q.LogWebhookArchiveEvent(ctx, exampleInput.BelongsToUser, exampleInput.ID)

...
func (q *Sqlite) GetAllOAuth2Clients(ctx context.Context, resultChannel chan []types.OAuth2Client) error {
	...
	if err != nil {
		return fmt.Errorf("error fetching count of oauth2 clients: %w", err)
	}
...
func (q *Sqlite) GetTotalOAuth2ClientCount(ctx context.Context) (uint64, error) {
	var count uint64
...
func (q *Sqlite) buildGetOAuth2ClientsForUserQuery(userID uint64, forAdmin bool, filter *types.QueryFilter) (query string, args []interface{}) {
	where := squirrel.Eq{
		fmt.Sprintf("%s.%s", queriers.OAuth2ClientsTableName, queriers.ArchivedOnColumn):                  nil,
		fmt.Sprintf("%s.%s", queriers.OAuth2ClientsTableName, queriers.OAuth2ClientsTableOwnershipColumn): userID,
	}
...
func (q *Sqlite) GetOAuth2Clients(ctx context.Context, userID uint64, filter *types.QueryFilter) (*types.OAuth2ClientList, error) {

...
func TestMariaDB_BuildGetDelegatedClientQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "SELECT delegated_clients.id, delegated_clients.external_id, delegated_clients.name, delegated_clients.client_id, delegated_clients.hmac_key, delegated_clients.created_on, delegated_clients.last_updated_on, delegated_clients.archived_on, delegated_clients.belongs_to_user FROM delegated_clients WHERE delegated_clients.archived_on IS NULL AND delegated_clients.belongs_to_user = ? AND delegated_clients.client_id = ?"
		expectedArgs := []interface{}{
...
func TestMariaDB_BuildUpdateDelegatedClientQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "UPDATE delegated_clients SET client_id = ?, last_updated_on = UNIX_TIMESTAMP() WHERE archived_on IS NULL AND belongs_to_user = ? AND id = ?"
		expectedArgs := []interface{}{
			exampleDelegatedClient.ClientID,
			exampleDelegatedClient.BelongsToUser,

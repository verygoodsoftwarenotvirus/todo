...
func TestMariaDB_BuildCreateOAuth2ClientQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "INSERT INTO oauth2_clients (external_id,name,client_id,client_secret,scopes,redirect_uri,belongs_to_user) VALUES (?,?,?,?,?,?,?)"
		expectedArgs := []interface{}{
		...
		actualQuery, actualArgs := q.BuildCreateOAuth2ClientQuery(exampleInput)
...
func TestMariaDB_BuildArchiveOAuth2ClientQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "UPDATE oauth2_clients SET last_updated_on = UNIX_TIMESTAMP(), archived_on = UNIX_TIMESTAMP() WHERE archived_on IS NULL AND belongs_to_user = ? AND id = ?"
		expectedArgs := []interface{}{
			exampleOAuth2Client.BelongsToUser,
			exampleOAuth2Client.ID,
		}

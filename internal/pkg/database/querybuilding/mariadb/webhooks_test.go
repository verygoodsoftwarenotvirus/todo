...
func TestMariaDB_BuildCreateWebhookQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedArgs := []interface{}{
			...
			exampleInput.URL,
			exampleInput.Method,
			strings.Join(exampleInput.Events, querybuilding.WebhooksTableEventsSeparator),
			strings.Join(exampleInput.DataTypes, querybuilding.WebhooksTableDataTypesSeparator),
			strings.Join(exampleInput.Topics, querybuilding.WebhooksTableTopicsSeparator),

...
func (s *service) NewTOTPSecretHandler(res http.ResponseWriter, req *http.Request) {
	...
	user, httpStatus := s.validateCredentialChangeRequest(
		ctx,
		sessionCtxData.Requester.RequestingUserID,
	...
	tracing.AttachRequestingUserIDToSpan(span, sessionCtxData.Requester.RequestingUserID)
	tracing.AttachUsernameToSpan(span, user.Username)
...
func (s *service) AvatarUploadHandler(res http.ResponseWriter, req *http.Request) {
	...
	user, err := s.userDataManager.GetUser(ctx, sessionCtxData.Requester.RequestingUserID)
	if err != nil {
	...
	internalPath := fmt.Sprintf("avatar_%d", sessionCtxData.Requester.RequestingUserID)

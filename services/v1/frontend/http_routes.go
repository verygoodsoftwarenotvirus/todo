...
<head>
    <meta charset="utf-8">
	<script>
	// copied from https://github.com/golang/go/blob/14491a2ec499668388d7fe7e95b84dbf0e1cf04d/misc/wasm/wasm_exec.js minified
...
<body>
    <script>
        const go = new Go();
        WebAssembly.instantiateStreaming(fetch("website.wasm"), go.importObject).then(result => {
...
var renderedHomePage []byte

func init() {
	renderedHomePage = []byte(fmt.Sprintf(homePage, "`${key}=${this.env[key]}`"))
}

// HomePage serves our homepage
func (s *Service) HomePage(res http.ResponseWriter, req *http.Request) {
	res.Write(renderedHomePage)
}

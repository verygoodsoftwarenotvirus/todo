...
func TestRegistrationFlow(T *testing.T) {
	...
	runTestOnAllSupportedBrowsers(T, func(driver selenium.WebDriver) func(t *testing.T) {
		return func(t *testing.T) {
			...
			require.NoError(t, passwordFieldFindErr, "unexpected error finding registration page password input field: %v", passwordFieldFindErr)
			registrationPagePasswordFieldInputErr := passwordField.SendKeys(user.Password)
			require.NoError(t, registrationPagePasswordFieldInputErr, "unexpected error sending input to registration page password input field: %v", registrationPagePasswordFieldInputErr)

			t.Logf("found registration page password field and filled it with: %q", user.Password)
			...
			require.NoError(t, passwordRepeatFieldFindErr, "unexpected error finding password repeat input field: %v", passwordRepeatFieldFindErr)
			...
			t.Logf("found registration page password repeat field and filled it with: %q", user.Password)
			...
			// fetch the password field and fill it
			passwordField, passwordFieldFindErr = driver.FindElement(selenium.ByID, passwordInputID)
			require.NoError(t, passwordFieldFindErr, "unexpected error finding login page password input field: %v", passwordFieldFindErr)
			...
			t.Logf("found login page password field and filled it with: %q", user.Password)
